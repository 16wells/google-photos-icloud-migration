#!/usr/bin/env python3
"""
Verify Google Drive OAuth credentials setup (for local use).

This tool verifies that your OAuth credentials are properly configured
to access Google Drive API from your local Mac. It checks:
- credentials.json file validity
- token.json file validity  
- Connection to Google Drive API

Note: This is for local macOS use only. No cloud deployment involved.
"""
import os
import sys
import json
...
def check_credentials_file(credentials_file: str = "credentials.json") -> Dict[str, Any]:
    ...
    print("=" * 60)
    print("Google Drive OAuth Credentials Check")
    print("=" * 60)
    print()
...
def main():
    ...
    if args.test_connection:
        if connection_ok:
            ...
            print()
            print("🎉 Your Google Drive OAuth setup is working correctly!")
        else:
