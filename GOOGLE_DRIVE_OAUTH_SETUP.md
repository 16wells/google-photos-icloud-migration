# Google Drive OAuth Credentials Setup Guide

**Note:** This tool runs **locally on macOS only**. This guide is about setting up OAuth credentials to authenticate with Google Drive API (not about deploying to the cloud).

This guide explains how to set up Google Cloud OAuth credentials (sometimes called "cloud agents") for the Google Photos to iCloud migration tool running **locally on your Mac**.

## What Are "Cloud Agents"?

"Cloud agents" in this context refers to **Google Cloud OAuth credentials** - the authentication mechanism that allows the tool to access your Google Drive from your local Mac to download Google Takeout zip files. These credentials are used locally; no cloud deployment is involved.
...
```bash
python3 verify-oauth-setup.py --test-connection
```
...
2. Invalid credentials
   - Verify credentials file: `python3 verify-oauth-setup.py`
...
Once your OAuth credentials are set up:
